    ...
    <el-dialog title="权限详情" v-model="viewDialogVisible" width="600px">
      <div v-if="viewData" class="detail-content">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="权限名称">{{ viewData.name }}</el-descriptions-item>
          <el-descriptions-item label="权限编码">{{ viewData.code }}</el-descriptions-item>
          <el-descriptions-item label="创建时间">{{ formatDateTime(viewData.createdTime) }}</el-descriptions-item>
          <el-descriptions-item label="更新时间">{{ formatDateTime(viewData.updatedTime) }}</el-descriptions-item>
          <el-descriptions-item label="描述" span="2">
            {{ viewData.description || '暂无描述' }}
          </el-descriptions-item>
          <el-descriptions-item label="关联角色" span="2">
            <el-tag v-for="role in viewData.roles" :key="role.id" style="margin-right: 8px; margin-bottom: 8px;">
              {{ role.name }}
            </el-tag>
            <span v-if="!viewData.roles.length" style="color: #999;">暂无关联角色</span>
          </el-descriptions-item>
        </el-descriptions>
      </div>
    </el-dialog>
...
const fetchPermissions = async () => {
  try {
    loading.value = true
    const response = await permissionApi.findPage(pagination.currentPage, pagination.pageSize, searchForm)
