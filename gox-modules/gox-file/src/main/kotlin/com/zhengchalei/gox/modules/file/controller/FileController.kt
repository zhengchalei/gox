...
class FileController(
    ...
    fun findByStorageName(
        @Parameter(description = "文件存储名称", required = true)
    ...
    fun search(
        ...
        @RequestParam("name") originalName: String
    ...
    fun findByStorageType(
        @Parameter(description = "存储类型", required = true)
        @PathVariable storageType: StorageType
    ): ResponseEntity<List<FileInfoListDTO>> {
    ...
    fun getTemporaryUrl(
        ...
        @RequestParam("minutes", defaultValue = "5") minutes: Long
    ): ResponseEntity<Map<String, String>> {
        return try {
            val fileInfo = fileService.findById(id)
    ...
    fun download(
        ...
        return try {
            ...
            if (fileInfo.storageType == StorageType.LOCAL) {
                ...
                val encodedFilename = URLEncoder.encode(fileInfo.originalName, StandardCharsets.UTF_8.toString())
                    .replace("+", "%20")
                ...
                ResponseEntity.status(302)
                    .header(HttpHeaders.LOCATION, tempUrl)
                    .build()
            }
        } catch (e: IllegalArgumentException) {
            ResponseEntity.notFound().build()
